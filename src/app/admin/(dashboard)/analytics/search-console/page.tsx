...
export default async function SearchConsolePage() {
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <h1 className="text-[24px] font-bold tracking-tight" style={{ color: "var(--a-text-1)" }}>Search Console</h1>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-[24px] font-bold tracking-tight" style={{ color: "var(--a-text-1)" }}>Search Console</h1>
        <p className="mt-1 text-[13px]" style={{ color: "var(--a-text-3)" }}>Google Search Performance data.</p>
      </div>
      ...
      {!snapshots?.length ? (
        <div className="a-card p-16 text-center">
          ...
          <p className="text-[14px] font-medium mb-1" style={{ color: "var(--a-text-2)" }}>No Search Console data</p>
          <p className="text-[13px]" style={{ color: "var(--a-text-4)" }}>Data will appear here after GA4/GSC sync is configured.</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {stats.map((s) => (
              <div key={s.label} className="a-card p-5">
                <p className="text-[12px] font-medium mb-2" style={{ color: "var(--a-text-3)" }}>{s.label}</p>
                <p className="text-[28px] font-bold tracking-tight" style={{ color: "var(--a-text-1)" }}>{s.value}</p>
              </div>
            ))}
          </div>

          <div className="a-card p-5">
            <h2 className="text-[13px] font-semibold mb-4" style={{ color: "var(--a-text-2)" }}>Impressions Over Time</h2>
          ...
          <div className="a-card overflow-hidden">
            <div className="px-4 py-3 border-b" style={{ borderColor: "var(--a-border)" }}>
              <h2 className="text-[13px] font-semibold" style={{ color: "var(--a-text-2)" }}>Daily Breakdown</h2>
            </div>
            <table className="w-full">
              ...
              <tbody>
                {snapshots.slice(0, 15).map((s, i) => (
                  <tr key={s.id} className="a-hover transition-colors duration-100"
                    style={{ borderBottom: i < 14 ? "1px solid var(--a-border)" : "none" }}>
                    <td className="px-4 py-3">
                      <span className="text-[13px]" style={{ color: "var(--a-text-1)" }}>
                        {new Date(s.date).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-[13px]" style={{ color: "var(--a-text-2)" }}>{(s.impressions || 0).toLocaleString()}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-[13px]" style={{ color: "var(--a-text-2)" }}>{(s.clicks || 0).toLocaleString()}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-[13px]" style={{ color: "var(--a-text-2)" }}>{((s.ctr || 0) * 100).toFixed(1)}%</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-[13px]" style={{ color: "var(--a-text-2)" }}>{(s.avg_position || 0).toFixed(1)}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
