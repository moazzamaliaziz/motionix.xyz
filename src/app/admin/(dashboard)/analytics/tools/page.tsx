...
export default async function ToolAnalyticsPage() {
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <h1 className="text-[24px] font-bold tracking-tight" style={{ color: "var(--a-text-1)" }}>Tool Analytics</h1>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-[24px] font-bold tracking-tight" style={{ color: "var(--a-text-1)" }}>Tool Analytics</h1>
        <p className="mt-1 text-[13px]" style={{ color: "var(--a-text-3)" }}>{events?.length || 0} events recorded</p>
      </div>
      ...
      {!events?.length ? (
        <div className="a-card p-16 text-center">
          ...
          <p className="text-[14px] font-medium mb-1" style={{ color: "var(--a-text-2)" }}>No tool usage data</p>
          <p className="text-[13px]" style={{ color: "var(--a-text-4)" }}>Data will appear here once tools are used and events are tracked.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {toolStats.map((tool) => (
            <div key={tool.slug} className="a-card p-5">
              <div className="flex items-start justify-between mb-3">
                <h3 className="text-[14px] font-semibold" style={{ color: "var(--a-text-1)" }}>{tool.slug}</h3>
              ...
              <div className="space-y-2 text-[12px]">
                <div className="flex justify-between">
                  <span style={{ color: "var(--a-text-4)" }}>Starts</span>
                  <span style={{ color: "var(--a-text-1)" }}>{tool.starts}</span>
                </div>
                <div className="flex justify-between">
                  <span style={{ color: "var(--a-text-4)" }}>Completions</span>
                  <span style={{ color: "var(--a-success)" }}>{tool.completes}</span>
                </div>
                <div className="flex justify-between">
                  <span style={{ color: "var(--a-text-4)" }}>Errors</span>
                  <span style={{ color: tool.errors > 0 ? "var(--a-error)" : "var(--a-text-4)" }}>{tool.errors}</span>
                </div>
                <div className="flex justify-between">
                  <span style={{ color: "var(--a-text-4)" }}>Avg Time</span>
                  <span style={{ color: "var(--a-text-2)" }}>{tool.avgMs > 0 ? `${(tool.avgMs / 1000).toFixed(1)}s` : "—"}</span>
                </div>
              </div>
            </div>
          ))}
