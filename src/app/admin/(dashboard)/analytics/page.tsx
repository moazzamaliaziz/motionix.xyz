...
export default async function AnalyticsOverviewPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-[24px] font-bold tracking-tight" style={{ color: "var(--a-text-1)" }}>Analytics</h1>
        <p className="mt-1 text-[13px]" style={{ color: "var(--a-text-3)" }}>Last 30 days of performance data.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {stats.map((s) => (
          <div key={s.label} className="a-card-interactive p-5">
            <p className="text-[12px] font-medium mb-2" style={{ color: "var(--a-text-3)" }}>{s.label}</p>
            <p className="text-[30px] font-bold tracking-tight" style={{ color: "var(--a-text-1)" }}>{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="a-card p-5">
        <h2 className="text-[13px] font-semibold mb-4" style={{ color: "var(--a-text-2)" }}>Traffic Trend</h2>
        {snapshots && snapshots.length > 0 ? (
          ...
          <div className="h-48 flex items-center justify-center">
            <p className="text-[13px]" style={{ color: "var(--a-text-4)" }}>No data yet.</p>
          </div>
        )}
      </div>
      ...
      {topTools.length > 0 && (
        <div className="a-card p-5">
          <h2 className="text-[13px] font-semibold mb-4" style={{ color: "var(--a-text-2)" }}>Top Tools</h2>
          <div className="space-y-3">
            {topTools.map(([slug, count]) => (
              <div key={slug} className="flex items-center gap-4">
                <span className="text-[13px] w-44 truncate" style={{ color: "var(--a-text-2)" }}>{slug}</span>
                <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: "var(--a-bg-elevated)" }}>
                ...
                <span className="text-[12px] w-16 text-right" style={{ color: "var(--a-text-3)" }}>{count} uses</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {!snapshots?.length && topTools.length === 0 && (
        <div className="a-card p-16 text-center">
          ...
          <p className="text-[14px] font-medium mb-1" style={{ color: "var(--a-text-2)" }}>No analytics data yet</p>
          <p className="text-[13px]" style={{ color: "var(--a-text-4)" }}>Data will appear once tools are used.</p>
        </div>
      )}
    </div>
  );
