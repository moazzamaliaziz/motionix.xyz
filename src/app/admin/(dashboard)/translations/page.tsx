  ...
  if (error) {
    return (
      <div className="space-y-6">
        <h1 className="text-[24px] font-bold tracking-tight text-[var(--a-text-1)]">Translations</h1>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-[24px] font-bold tracking-tight text-[var(--a-text-1)]">Translations</h1>
          <p className="mt-1 text-[13px] text-[var(--a-text-3)]">Locale completeness across all pages.</p>
        </div>
      ...
      {locales.length === 0 ? (
        <div className="admin-card p-16 text-center">
          ...
          <p className="text-[14px] font-medium mb-1 text-[var(--a-text-2)]">No translation data</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {locales.map((loc) => {
            ...
            return (
              <div key={loc.locale} className="admin-card-hover p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-[15px] font-bold uppercase tracking-tight text-[var(--a-text-1)]">{loc.locale}</h3>
                  <span className="text-[12px] font-semibold" style={{ color: textColor }}>{loc.pct}%</span>
                </div>
                <div className="w-full h-1.5 rounded-full mb-4 overflow-hidden bg-[var(--a-bg-elevated)]">
                  <div className="h-full rounded-full transition-all duration-500" style={{ width: `${loc.pct}%`, background: barColor }} />
                </div>
                <div className="space-y-2 text-[12px]">
                  <div className="flex justify-between"><span className="text-[var(--a-text-4)]">Total pages</span><span className="text-[var(--a-text-2)]">{loc.total}</span></div>
                  <div className="flex justify-between"><span className="text-[var(--a-text-4)]">Indexable</span><span className="text-[var(--a-success)]">{loc.indexable}</span></div>
                  <div className="flex justify-between"><span className="text-[var(--a-text-4)]">SEO complete</span><span className="text-[var(--a-text-3)]">{loc.seoComplete}</span></div>
                </div>
              </div>
            );
          })}
