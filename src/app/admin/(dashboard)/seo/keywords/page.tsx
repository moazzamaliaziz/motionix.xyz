  ...
  if (error) {
    return (
      <div className="space-y-6">
        <h1 className="text-[24px] font-bold tracking-tight" style={{ color: "var(--a-text-1)" }}>Keywords</h1>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-[24px] font-bold tracking-tight" style={{ color: "var(--a-text-1)" }}>Keywords</h1>
          <p className="mt-1 text-[13px]" style={{ color: "var(--a-text-3)" }}>{keywords?.length || 0} tracked keywords</p>
        </div>
      ...
      {!keywords?.length ? (
        ...
        <div className="a-card overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {keywords.map((kw, i) => (
                <tr key={kw.id} className="a-hover transition-colors duration-100"
                  style={{ borderBottom: i < keywords.length - 1 ? "1px solid var(--a-border)" : "none" }}>
                  <td className="px-4 py-3">
                    <div>
                      <p className="text-[13px] font-medium" style={{ color: "var(--a-text-1)" }}>{kw.keyword}</p>
                      {kw.target_url && <p className="text-[11px] mt-0.5" style={{ color: "var(--a-text-4)" }}>{kw.target_url}</p>}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-[13px] font-medium" style={{ color: "var(--a-text-1)" }}>
                  ...
                  <td className="px-4 py-3">
                    <span className="text-[13px]" style={{ color: kw.current_rank ? "var(--a-text-1)" : "var(--a-text-4)" }}>
                      {kw.current_rank ? `#${kw.current_rank}` : "—"}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-[13px]" style={{ color: kw.target_rank ? "var(--a-accent)" : "var(--a-text-4)" }}>
                      {kw.target_rank ? `#${kw.target_rank}` : "—"}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <span className="text-[12px]" style={{ color: "var(--a-text-4)" }}>{kw.country || "—"}</span>
                  </td>
                </tr>
              ))}
            </tbody>
...
function DifficultyBar({ value }: { value: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="w-16 h-1.5 rounded-full overflow-hidden" style={{ background: "var(--a-bg-elevated)" }}>
