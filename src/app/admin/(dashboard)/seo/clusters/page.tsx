  ...
  if (error) {
    return (
      <div className="space-y-6">
        <h1 className="text-[24px] font-bold tracking-tight" style={{ color: "var(--a-text-1)" }}>Topic Clusters</h1>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-[24px] font-bold tracking-tight" style={{ color: "var(--a-text-1)" }}>Topic Clusters</h1>
          <p className="mt-1 text-[13px]" style={{ color: "var(--a-text-3)" }}>{clusters?.length || 0} clusters</p>
        </div>
      ...
      {!clusters?.length ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {clusters.map((cluster) => (
            <div key={cluster.id} className="a-card-interactive p-5">
              <div className="flex items-start justify-between mb-3">
                <h3 className="text-[15px] font-semibold" style={{ color: "var(--a-text-1)" }}>{cluster.name}</h3>
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                  cluster.status === "active" ? "bg-emerald-500/10 text-emerald-400" : "bg-white/10 text-white/40"
                }`}>
                  {cluster.status}
                </span>
              </div>
              <p className="text-[12px] mb-3" style={{ color: "var(--a-text-3)" }}>{cluster.description || "No description"}</p>
              <div className="space-y-1.5 text-[12px]">
                {cluster.pillar_slug && (
                  <div className="flex justify-between">
                    <span style={{ color: "var(--a-text-4)" }}>Pillar</span>
                    <span style={{ color: "var(--a-text-2)" }}>/{cluster.pillar_slug}</span>
                  </div>
                )}
                {cluster.tool_slug && (
                  <div className="flex justify-between">
                    <span style={{ color: "var(--a-text-4)" }}>Tool</span>
                    <Link href={`/admin/tools/${cluster.tool_slug}`} className="hover:underline" style={{ color: "var(--a-accent)" }}>
                      {cluster.tool_slug}
                    </Link>
                  </div>
                )}
