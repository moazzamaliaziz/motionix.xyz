  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-[24px] font-bold tracking-tight text-[var(--a-text-1)]">SEO Manager</h1>
          <p className="mt-1 text-[13px] text-[var(--a-text-3)]">Search engine optimization tools and tracking.</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {stats.map((stat) => (
          <Link key={stat.label} href={stat.href} className="admin-card-hover p-4 group block">
            <p className="text-[12px] font-medium mb-2 text-[var(--a-text-3)]">{stat.label}</p>
            <div className="flex items-baseline gap-2">
              <span className="text-[28px] font-bold tracking-tight text-[var(--a-text-1)]">{stat.value}</span>
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"
                className="transition-opacity duration-150 group-hover:opacity-100 text-[var(--a-text-4)] opacity-[0.3]">
                <path d="M4 2l4 4-4 4" />
              </svg>
            </div>
          </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {sections.map((s) => (
          <Link key={s.href} href={s.href} className="admin-card-hover p-5 group block">
            <h3 className="text-[14px] font-semibold group-hover:opacity-80 transition-opacity duration-100 text-[var(--a-text-1)]">{s.title}</h3>
            <p className="text-[12px] mt-1 leading-relaxed text-[var(--a-text-3)]">{s.desc}</p>
          </Link>
        ))}
      </div>
    </div>
