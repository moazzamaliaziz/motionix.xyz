  ...
  if (error) {
    return (
      <div className="space-y-6">
        <h1 className="text-[24px] font-bold tracking-tight text-[var(--a-text-1)]">SEO Issues</h1>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-[24px] font-bold tracking-tight text-[var(--a-text-1)]">SEO Issues</h1>
          <p className="mt-1 text-[13px] text-[var(--a-text-3)]">{unresolved.length} unresolved · {resolved.length} resolved</p>
        </div>
      ...
      {!issues?.length ? (
        ...
        <div className="space-y-4">
          {unresolved.length > 0 && (
            <div className="admin-card overflow-hidden">
              <div className="px-4 py-3 border-b border-[var(--a-border)]">
                <h2 className="text-[13px] font-semibold text-[var(--a-text-2)]">Unresolved</h2>
              </div>
              <table className="w-full">
                ...
                <tbody>
                  {unresolved.map((issue, i) => (
                    <tr key={issue.id} className="admin-hover transition-colors duration-100"
                      style={{ borderBottom: i < unresolved.length - 1 ? "1px solid var(--a-border)" : "none" }}>
                      <td className="px-4 py-3">
                        <SeverityBadge severity={issue.severity} />
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-[12px] text-[var(--a-text-2)]">{issue.issue_type}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-[12px] font-mono truncate block max-w-[200px] text-[var(--a-accent)]">{issue.page_url}</span>
                      </td>
                      <td className="px-4 py-3">
          ...
          {resolved.length > 0 && (
            <div className="admin-card overflow-hidden">
              <div className="px-4 py-3 border-b border-[var(--a-border)]">
                <h2 className="text-[13px] font-semibold text-[var(--a-text-2)]">Resolved</h2>
              </div>
              <table className="w-full">
                <tbody>
                  {resolved.map((issue, i) => (
                    <tr key={issue.id} className="admin-hover transition-colors duration-100"
                      ...
                      <td className="px-4 py-3">
                        <span className="text-[12px] text-[var(--a-text-3)]">{issue.issue_type}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-[12px] font-mono truncate block max-w-[200px] text-[var(--a-text-4)]">{issue.page_url}</span>
                      </td>
                      <td className="px-4 py-3 text-right">
                        <span className="text-[11px] bg-[var(--a-success)]/15 text-[var(--a-success)] px-2 py-0.5 rounded-full">Resolved</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
...
function EmptyState({ icon, title, hint }: { icon: string; title: string; hint: string }) {
  return (
    <div className="admin-card p-16 text-center">
      ...
      <p className="text-[13px] text-[var(--a-text-4)]">{hint}</p>
    </div>
  );
}
