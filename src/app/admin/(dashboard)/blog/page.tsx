      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-2">
          <Link href="/admin/blog/new"
            className="a-btn a-focus px-4 py-2 bg-white text-black rounded-md text-[13px] font-semibold hover:bg-white/90 transition-colors duration-100">
            New Post
          </Link>
        </div>
      </div>
      ...
      {!posts?.length ? (
        <div className="a-card p-16 text-center">
          ...
          <p className="text-[13px]" style={{ color: "var(--a-text-4)" }}>
            Run <code className="px-1.5 py-0.5 rounded text-[12px]" style={{ background: "var(--a-bg-elevated)", color: "var(--a-text-3)" }}>supabase/seed.sql</code> or click &quot;New Post&quot; to create one.
          </p>
        </div>
      ) : (
