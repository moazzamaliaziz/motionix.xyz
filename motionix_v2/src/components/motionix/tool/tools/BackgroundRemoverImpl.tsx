...
export function BackgroundRemoverImpl() {
  ...
  // Pristine transparent cutout from the AI (or the last refine export). Set ONCE per result and
  // never mutated by applyBackground, so every recomposition starts from the original cutout.
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (outUrl) URL.revokeObjectURL(outUrl);
      // Store the pristine transparent cutout ONCE; applyBackground always recomposes from this.
  ...
  // Explicit overrides make recomposition deterministic even when called from a
  // setTimeout whose closure captured stale state values.
  const applyBackground = async (colorArg?: string, opacityArg?: number, sizeArg?: number) => {
    // Always recompose from the pristine transparent cutout, never from the last output.
    const cutout = cutoutBlobRef.current;
    if (!cutout) return;

    const color = colorArg ?? bgColor;
    const opacity = opacityArg ?? shadowOpacity;
    const size = sizeArg ?? shadowSize;
    const isTransparent = color === "transparent";

    // Transparent = the pristine cutout itself. Restore it as the output.
    if (isTransparent) {
      if (outUrl) URL.revokeObjectURL(outUrl);
      setOutBlob(cutout);
      setOutUrl(URL.createObjectURL(cutout));
      return;
    }

    const probeUrl = URL.createObjectURL(cutout);
    ...
    const hasShadow = opacity > 0;

    const main = document.createElement("canvas");
    main.width = w;
    main.height = h + (hasShadow ? Math.round(h * 0.12 * size) : 0);
    ...
    if (hasShadow) {
      ...
      if (filtered.length >= 2) {
        const shadowY = h + Math.round(h * 0.06 * size);
        const blurPx = Math.round(Math.min(w, h) * 0.04 * size);
        ...
        for (let i = filtered.length - 1; i >= 0; i--) {
          ctx.lineTo(filtered[i].x, shadowY - size * 6);
        }
        ctx.closePath();
        ctx.fillStyle = "#000";
        ctx.fill();
    ...
    <div className="space-y-5">
      {outUrl && refineMode && imgDimensions.w > 0 ? (
        <RefineCanvas
          ...
          onExport={(blob) => {
            if (outUrl) URL.revokeObjectURL(outUrl);
            // The refined result is the new pristine cutout; recompositions start from it.
            cutoutBlobRef.current = blob;
            setOutBlob(blob);
            setOutUrl(URL.createObjectURL(blob));
              ...
              <div className="flex flex-wrap gap-2">
                {bgChoices.map((c) => (
                  <button
                    ...
                    onClick={() => {
                        setBgColor(c.token);
                        // Recompose from the pristine cutout with the chosen color. Transparent
                        // restores the pristine cutout as output.
                        setTimeout(() => applyBackground(c.token), 0);
                      }}
                  ...
                  <label className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        // v === 0 recomposes with no shadow (still valid, not a no-op).
                        setTimeout(() => applyBackground(bgColor, v, shadowSize), 0);
                      }}
                  ...
                  <label className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const v = Number(e.target.value);
                        setShadowSize(v);
                        setTimeout(() => applyBackground(bgColor, shadowOpacity, v), 0);
                      }}
                      className="accent-primary w-full"
                    />
                  </label>
